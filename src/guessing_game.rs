...
pub fn guess() {
    ...
    let secret_num: u32 = rand::thread_rng().gen_range(1..=100); // Generate a random integer from 1-100.

    loop {
        println!("Please input your guess.");

        /* Variables are immutable by default in rust, and require the mut tag to become mutable. */
        let mut guess: String = String::new(); // Since String isn't a primitive type, it must be initialized via "String::new();".
        ...
        if guess.contains("q") || guess.contains("Q") {
            // Add way to exit the gameplay loop by typing "quit".
            break;
        }

        let guess: u32 = match guess.trim().parse() {
            // Parse the guess String into an unsigned 32-bit number.
        ...
        match guess.cmp(&secret_num) {
            // Matches are the same as switches in C-based languages.
            Ordering::Less => println!("Too small!"),
            Ordering::Greater => println!("Too big!"),
            Ordering::Equal => {
                // If the guess is the same as the secret number, print "You win!", then break the loop (ending the game).
                println!("YOU WIN!");
                break;
            }
        }
